...
namespace ItAcademy.Infrastructure.Orders
{
    public class OrderRepository : IOrderRepository
    {
        ...
        public OrderRepository(IBaseRepository<Order> orderRepository, IBaseRepository<Event> eventRepository, IBaseRepository<AppUser> userRepository)
        {
        ...
        {
            var order = await _orderRepository.GetQuery(i => i.Id == id).SingleOrDefaultAsync().ConfigureAwait(false);
            if (order == null)
        ...
        public async Task<bool> AddOrder(CancellationToken token, OrderRequest order, int eventId)
        {
            ...
            var user = await _userRepository.GetQuery(i => i.Id == order.UserId).SingleOrDefaultAsync().ConfigureAwait(false);
        ...
        {
            return await _orderRepository.GetQuery().Include(i => i.User).Include(i => i.Event).ToListAsync(token).ConfigureAwait(false);
        }
        public async Task<List<Order>> GetAllOrdersByIdAndRole(CancellationToken token, string userId, string userRole)
        {
            var orders = await _orderRepository.GetQuery().Include(i => i.User).Include(i => i.Event).ToListAsync(token).ConfigureAwait(false);
